...
export const TVShowDetailPage = () => {
  ...
  if (status === 'loading') return <div className="p-12 text-center text-gray-400">Loading TV show details...</div>;
  if (status === 'error' || !show) return <div className="p-12 text-center text-red-500">Show Not Found</div>;

  // Filter out season 0 (often used for specials) to keep the UI clean
  const standardSeasons = show.seasons?.filter(s => s.season_number > 0) || [];

  return (
    <div className="pb-20 relative">
      <TrailerModal isOpen={isTrailerOpen} videoKey={trailerKey} onClose={() => setIsTrailerOpen(false)} />

      {/* Hero Section (Identical structural layout to Movie Details) */}
      <div className="relative w-full h-[60vh] min-h-[500px]">
        <div className="absolute inset-0 z-0 opacity-30">
          <TMDBImage path={show.backdrop_path} type="backdrop" alt={show.name || 'Backdrop'} className="w-full h-full object-cover" />
        ...
        <div className="relative z-10 container mx-auto px-6 h-full flex flex-col md:flex-row items-end pb-12 gap-8">
          <div className="hidden md:block w-64 flex-shrink-0 rounded-xl overflow-hidden shadow-2xl border border-gray-700 mt-12">
            <TMDBImage path={show.poster_path} type="poster" alt={show.name || 'Poster'} className="w-full" />
          </div>

          <div className="flex-1">
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-2">{show.name}</h1>
            <div className="flex flex-wrap items-center gap-4 text-sm text-gray-300 mb-6">
              <span className="text-yellow-500 font-bold">★ {show.vote_average?.toFixed(1) || 'NR'}</span>
      ...
      <div className="container mx-auto px-6 mt-12">
        ...
        {/* React Router NavLinks automatically get an 'active' class when their URL matches! */}
        ...
        {/* The selected season's episodes will render exactly here! */}
